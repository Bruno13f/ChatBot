...
export const initBackendSocket = (userId: string, groupId: string, onNewMessage: (message: any) => void): Socket => {
  if (!socketInstance) {
    const SOCKET_URL = process.env.NEXT_PUBLIC_BACKEND_SOCKET_URI || 'http://localhost:9000';
