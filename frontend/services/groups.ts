...
export async function deleteGroupPicture(groupId: string) {
  if (typeof window === "undefined") return; // SSR guard
  ...
  const res = await fetch(`${BACKEND_URI}/groups/${groupId}/picture`, {
    method: "DELETE",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${token}`,
  ...
  const data = await res.json();
  if (!res.ok)
    throw new Error(data.message || "Failed to delete group picture.");
  return data.group;
}
...
export const addUsersToGroup = async (
  ...
  if (typeof window === "undefined")
    throw new Error("Cannot add users to group on server side");
  const token = localStorage.getItem("token");
  if (!token) {
    window.location.href = "/login";
...
export async function removeMemberFromGroup(groupId: string, memberId: string) {
  ...
  const res = await fetch(
    `${BACKEND_URI}/groups/${groupId}/members/${memberId}`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
